...
**What it returns:**
A `str` — a styling suggestion that references at least one wardrobe item by `name` (e.g. "Pair this with your baggy straight-leg jeans and chunky white sneakers…") followed by a concrete styling tip. Generated by the LLM (Groq `llama-3.3-70b-versatile`). The string is later passed unchanged into `create_fit_card`.

**What happens if it fails or returns nothing:**
If `wardrobe["items"]` is empty, the tool does NOT crash — it falls back to general styling advice for the item (what kinds of pieces pair well, what vibe it suits) instead of naming specific wardrobe items, and still returns a non-empty string. If the wardrobe has items but no strong match is found, the LLM returns a generic-but-valid suggestion rather than failing.
...
**Input parameters:**
- `outfit` (str): the styling suggestion returned by `suggest_outfit` — the text describing how to wear the item with the user's wardrobe.
...
**What happens if it fails or returns nothing:**
If `outfit` is empty or whitespace-only, the tool returns a descriptive error-message string (e.g. "Couldn't generate a fit card: no outfit suggestion was provided…") rather than raising. The planning loop can also skip the tool when the outfit is missing and still return the listing and styling suggestion gathered so far — it never returns a malformed or half-empty card.
...
- **suggest_outfit:** Input = the *Tool 2* block + the wardrobe schema fields. Ask Claude to implement it (calling Groq `llama-3.3-70b-versatile`) matching `new_item` against `wardrobe["items"]` by `category`/`colors`/`style_tags` and returning a suggestion string that names real items. *Verify:* run it with `get_example_wardrobe()` (expect named pieces) and with `get_empty_wardrobe()` (expect non-empty general advice, no crash).
...
Using the result from Step 1, the agent calls `suggest_outfit(new_item=<the baby tee>, wardrobe=get_example_wardrobe())`. The wardrobe already contains baggy jeans (`w_001`) and chunky white sneakers (`w_007`), so the tool returns a styling suggestion string that pairs the new tee with those pieces.

**Step 3 — Fit card:**
With the suggestion string from Step 2, the agent calls `create_fit_card(outfit=<suggestion>, new_item=<the baby tee>)`, which returns a short, casual social-media-style caption for the look.
